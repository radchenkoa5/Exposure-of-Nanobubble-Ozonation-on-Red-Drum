...
```{r}
mod1 <- lm(Chl.a1 ~ Tank + Treatment + Temp + 
                  Temp:Treatment + Time, 
              data = WQ)
summary(mod1)
```
...
```{r}
sim_mod <- lm(Chl.a1 ~ Tank + Treatment + Temp + Temp:Treatment, 
              data = WQ)
sub_mod <- update(sim_mod, . ~ . - Tank)
main_mod <- lm(Chl.a1 ~ Treatment, data = WQ)
summary(sim_mod)
plot(sim_mod)
...
shapiro.test(residuals(sim_mod))
```


```{r}
sim_mod <- lm(Chl.a ~ Tank + Treatment + Temp + Temp:Treatment, 
              data = WQ)
sub_mod <- update(sim_mod, . ~ . - Tank)
main_mod <- lm(Chl.a ~ Treatment, data = WQ)
summary(sim_mod)
plot(sim_mod)
...
```{r}
sim_mod <- lm(Chl.a ~ Tank + Treatment  + Tank:Treatment, 
              data = WQ)
